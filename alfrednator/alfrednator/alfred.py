...
class VrepSource(object):
    #TODO mudar tudo, pq n eh o ip que varia entre robos virtuais, eh o nome da cameras
    ...
    def __init__(self, virtual_robot_ip, virtual_robot_port):
        virtual_robot_ip = virtual_robot_ip.replace("vrep-", "")
        ...
        if not "vrep-" + self.virtual_robot_ip + "-" + str(self.virtual_robot_port) in source_dict:
            #new capture
            source_dict["vrep-" + self.virtual_robot_ip + "-" + str(self.virtual_robot_port)] = self
...
@SOCKETIO.on("start-vrep-video")
## \details Function to handle the request of a new virtual robot source. It renders the templates for the tabs and frames for the source and returns them.
# \brief Function to handle the request of a new virtual robot source.
# \param virtual_robot_path String: String containning the path selected by the text input on the top of the page.
# \param source_number String: The number that should be this new source.
# \param virtual_port int: Port number that vrep simulation is running on. Defaults to 19997.
def handle_start_vrep_video(virtual_robot_path, source_number, virtual_port=19997):

    #TODO mudar tudo, pq n eh o ip que varia entre robos virtuais, eh o nome da cameras
    source_path = virtual_robot_path + "-" + str(virtual_port)
    if not source_path in source_dict:
        VrepSource(virtual_robot_path, virtual_port)
    
    virtual_robot_path = virtual_robot_path + "-" + str(virtual_port)
    
    top_image_path = virtual_robot_path + "-top-image"
    ...
    bottom_image_path = virtual_robot_path + "-bottom-image"
    #generate bottom image source and tab
    bottom_image_tab = render_template("robot_frame_tab.html",
                                       source_number=int(source_number) + 1,
                                       path=bottom_image_path)
...
#event fired within the display loop
@SOCKETIO.on("request-vrep-img")
##  \details Funtion to handle the "request-vrep-img" event from socketio. Should be fired everytime the frontend needs a new virtual robot image.
#   \brief Function to handle "request-vrep-img" event from socketIO.
#   \param path String: String containning the path selected by the text input on the top of the page (with ip), in the form of "vrep-localhost-19997"
#   \param function_status Boolean/String:  Either false if there is no selected function or a string with the funtion name.
def handle_request_vrep_img(path, function_status, selected_index):
    try:
        top_img_byte_string, bottom_img_byte_string, debug_image_size = generate_vrep_images(
            path, function_status, selected_index)
    except RuntimeError as error:
        return "error " + error.message, path
    return top_img_byte_string, bottom_img_byte_string, path, debug_image_size

#Event fired when the user requests the split canvas function.
...
## \details Function called within a loop to get the virtual robot images.
#  \param   path String: source path, in the form of "vrep-localhost-199997".
...
#  \return Returns the byte string that represents the virtual robot images encoded in .jpg without the "data:image/jpeg;base64,".
def generate_vrep_images(path, function_name, selected_index):
    key_name = path

    virtual_robot_src = source_dict[key_name]

    top_frame = virtual_robot_src.get_new_top_image()
    top_encoded, debug_image_size = encode_image(top_frame, function_name,
                                                 selected_index)

    bottom_frame = virtual_robot_src.get_new_bottom_image()
